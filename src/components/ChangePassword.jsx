...
function ChangePassword() {
  ...
  const submitForm = async (event) => {
    ...
    if (currentPassword.value === newPasswordOne.value) {
      showAlert(
        "Error",
        "Your entered current password can't be same as your new password, please try again!"
      );
    ...
    if (!passwordRegex.test(newPasswordOne.value)) {
      showAlert(
        "Error",
    ...
    try {
      ...
      if (error.code === "auth/invalid-login-credentials") {
        showAlert(
          "Error",
          "The entered current password is incorrect. Please try again."
        );
      } else {
